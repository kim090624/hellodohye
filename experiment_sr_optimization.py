# -*- coding: utf-8 -*-
"""
[보고서 2단계 최적화 실험] SR 파라미터(a, b) 및 최적 노이즈(sigma) 산출 (지오폰 / 마이크 독립 튜닝 지원)
================================================================================
...
def load_latest_noise(sensor_suffix):
    ...
    # 하위 호환성 지원: 센서명이 명시되지 않은 파일(env_noise_*.csv)도 검색 대상에 포함
    ...
    if not step_files:
        ...
        if animal_signal is not None:
            if mode == '1':
                ...
                # 환경 소음을 별도로 더하지 않기 위해 0으로 채워진 배열을 전달
